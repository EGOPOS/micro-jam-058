...
@onready var drown_vingette: ColorRect = %DrownVingette
...
@onready var radar_rect: ColorRect = %RadarRect
...
func _ready() -> void:
	...
	radar_mat = radar_rect.material as ShaderMaterial
...
func _process(delta: float) -> void:
	if player:
		...
		set_vingete_value(drown_vingette.material, remap(player.drown_timer, 0, player.time_to_drown, 0, 1.0))
...
func update_dots() -> void:
	for dot_indx in dots.size():
		...
		dot.visible = not hit.is_empty() and visible
		right_pickaxe_dot.visible = player.is_right_side_enabled and not hit.is_empty() and visible
		if not hit.is_empty():
	...
	if backpack_comp:
		if backpack_comp.has_method("get_overweight"):
			...
			if mat:
				...
				set_vingete_value(mat, target)


func set_vingete_value(material, value):
	if material:
		var current = material.get_shader_parameter("fadeAmount")
		create_tween().tween_method(func(value):
			material.set_shader_parameter("fadeAmount", value)
		, current, value, 0.5).set_ease(Tween.EASE_IN_OUT)
